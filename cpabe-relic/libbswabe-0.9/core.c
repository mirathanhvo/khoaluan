...
bswabe_prv_t* bswabe_keygen(bswabe_pub_t* pub, bswabe_msk_t* msk, char** attributes) {
    ...
    for (int i = 0; attributes[i]; i++) {
        ...
        g2_mul(c.d, pub->gp, ri);

        g2_null(c.z); g2_new(c.z);
        g2_mul_gen(c.z, ri);

        g1_null(c.zp); g1_new(c.zp);
        g1_mul(c.zp, pub->h, ri);
    ...
    if(p->children->len == 0) {
        ...
        for (int i = 0; i < p->satl->len; i++) {
            ...
            // In giá trị intermediate của temp sau exponentiation
            {
                ...
                if (p->attr)
                    printf("[dec_node_naive] temp (attr = %s): ", p->attr);
                else
                    printf("[dec_node_naive] temp (internal node): ");
                for (int j = 0; j < size; j++) {
                    printf("%02x", buf[j]);
                }
                printf("\n");
...
// Hàm chuẩn hóa attr (loại bỏ ký tự \n và khoảng trắng đầu/cuối)
